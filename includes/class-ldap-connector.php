...
class LDAP_ED_Connector {
	...
	public function connect() {
		...
		if ( empty( $server ) ) {
			return new \WP_Error( 'ldap_no_server', __( 'LDAP server address is not configured.', 'ldap-employee-directory' ) );
		}
		...
		if ( ! $this->connection ) {
			return new \WP_Error( 'ldap_connect_failed', __( 'Could not create LDAP connection handle.', 'ldap-employee-directory' ) );
		}
	...
	public function bind() {
		...
		if ( ! $bound ) {
			...
			return new \WP_Error(
				...
				sprintf( __( 'LDAP bind failed: %s', 'ldap-employee-directory' ), $error )
			);
		}
	...
	public function get_users() {
		...
		if ( ! $search ) {
			...
			return new \WP_Error(
				'ldap_search_failed',
				/* translators: %s: LDAP error message */
				sprintf( __( 'LDAP search failed: %s', 'ldap-employee-directory' ), $error )
			);
		}
	...
	public function test_connection() {
		...
		return array(
			...
			'message' => sprintf( __( 'Connection successful. %d user(s) found.', 'ldap-employee-directory' ), count( $users ) ),
			'count'   => count( $users ),
		);
	}
